...
class TinyboardSpider(scrapy.Spider):
    ...
    Can be started on an homepage, catalog page, or a 
    specific thread URL.
    """
    name = 'tinyboard'
    ...
    def parse(self, response, **kwargs):
        ...
        if is_thread_page:
            ...
            # TODO: Add handling of board pages
            # # Also, look for a "Next" button to handle pagination
    ...
    def _parse_thread_page(self, response):
        ...
        thread_loader.add_css(
            'thread_id', 'div[id^="thread_"]::attr(id)', re=r'(\d+)'
        )
        thread_loader.add_css('board', 'header h1::text', re=r'/(.*?)/')
        thread_loader.add_css('title', 'title::text', re=r' - (.*)')
    ...
    def _parse_post(self, post_selector, response):
        ...
        post_loader.add_value('is_op', is_op)

        # Scrape standard post fields
        post_loader.add_css('post_id', 'a.post_no::text', re=r'(\d+)')
        post_loader.add_css('author', 'span.name::text')
        post_loader.add_css('timestamp', 'time::attr(datetime)')
        post_loader.add_css('content', 'div.body')
        post_loader.add_css(
            'linked_to_ids', 'div.body a::text', re=r'>>(?:>)?/?(.+)'
        )

        # !! Image URL Logic !!
        # Look for the image link inside the post div first
        image_css = 'div.files a::attr(href), p.fileinfo a::attr(href)'
        ...
        if relative_url:
            ...
            post_loader.add_value('image_url', full_url)
